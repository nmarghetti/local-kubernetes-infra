...
build_helm() {
  ...
  chart_name=$(yq '.name' <"$GIT_ROOT"/helm/Chart.yaml)
  chart_version=$(yq '.version' <"$GIT_ROOT"/helm/Chart.yaml)
  ...
  run_command_hide helm lint "$GIT_ROOT"/helm || exit_error "Helm lint failed"
  run_command_hide helm unittest "$GIT_ROOT"/helm || exit_error "Helm unittest failed"
  run_command_hide helm package "$GIT_ROOT"/helm -d "$GIT_ROOT"/tmp || exit_error "Helm build failed"
